          ...
          switch (UsbSetupBuf->wValueH) {
            ...
            case 0x22:
              if (UsbSetupBuf->wValueL == 0) {
                ...
                len = ConfigurationDescriptor.HID_Device.HIDReportLength;
              } else {
                len = 0xff;
              }
              break;
...
void USBInterrupt(void) {  // inline not really working in multiple files in SDCC
  ...
  if (UIF_BUS_RST) {
    ...
    UEP2_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK | UEP_R_RES_ACK;  // Endpoint 2 automatically flips the sync flag,
...
void USBDeviceEndPointCfg() {
  ...
  UEP2_3_MOD =
    bUEP3_RX_EN | bUEP3_TX_EN | bUEP2_RX_EN | bUEP2_TX_EN;  // Endpoint2 double buffer
  UEP1_CTRL =
  ...
  UEP3_CTRL = bUEP_AUTO_TOG | UEP_T_RES_NAK | UEP_R_RES_ACK;  // Endpoint 3 automatically flips the sync flag, IN
                                                              // transaction returns NAK, OUT returns ACK
